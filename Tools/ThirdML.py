...
# cross validation helps evaluate model performance more robustly since it test and trains on multiple splits of the data
# Use 5-fold cross-validation
cv_scores = cross_val_score(rf, x, y, cv=5, scoring='accuracy')  
# rf: the model
# x, y: full dataset
# cv=5: splits the dataset into 5 folds 80% ~ traiing, 20% ~ testing, then rotat
# scoring='accuracy': evaluates performance using accuracy

print("Cross-validation scores:", cv_scores)  # prints accuracy for each fold
print("Mean CV Accuracy:", cv_scores.mean())  # average accuracy across all folds
"""

"""
# StandardScaler - scales the data so each feature has mean=0 and std=1    probably wont need this for RF
# useful because some ML models perform better when inputs are normalized,  x_scaled = (x - mean) / std
from sklearn.preprocessing import StandardScaler

scaler = StandardScaler()                  # create scaler object
scaler.fit(x_train)                        # calcs mean and std ONLY from training data
x_train_scaled = scaler.transform(x_train) # scale the training data using mean and std from scaler.fit, ~ centered at 0 std=1
x_test_scaled  = scaler.transform(x_test)  # scale the test data using same scaling 

# Train RF model using the scaled data
rf_scaled = RandomForestClassifier(n_estimators=100, random_state=42) # initialize RF model for the scaled data
rf_scaled.fit(x_train_scaled, y_train)                                # train the model on scaled training data

y_pred_scaled = rf_scaled.predict(x_test_scaled)                      # predictions using scaled data
accuracy_scaled = accuracy_score(y_test, y_pred_scaled)               # accuracy calculation
